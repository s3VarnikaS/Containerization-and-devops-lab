...
Users write playbooks with tasks and modules to define the desired state of the system. The managed machines are represented in a simplistic **inventory** file that groups all the nodes into different categories. Ansible leverages YAML to define playbooks in a human-readable data format.
...
- Includes sophisticated features around modularity and reusability
- Extensive and comprehensive official documentation complemented by community material

> *To sum up, Ansible is simple yet powerful, agentless, community-powered, predictable, and secure.*

---

## Part A: Ansible Installation Instructions
...
*First create SSH key-pair and then create custom ubuntu-server image with openssh configured.*
...
Using docker image `ubuntu-server` created in previous part, run 4 test servers named server1 to server4.
...
The below script will create and update `inventory.ini` with updated docker container IPs. Review this file and create your own if needed.
...
> *The workflow is now: 1. Setup SSH keys → 2. Build image → 3. Launch containers → 4. Create inventory → 5. Test → 6. Run playbook → 7. Verify → 8. Cleanup*
...
If you want a clean slate for testing or isolated environments, use Docker to run containers:
